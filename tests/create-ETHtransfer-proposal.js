...
async function testCreateETHProposal() {
    ...
    const ETH_AMOUNT = ethers.parseEther("0.0001"); // 0.0001 ETH
    ...
    // Create execution data for ETH transfer
    const iface = new ethers.Interface([
        "function transfer(address _to) external payable"
    ]);
    
    const executionData = iface.encodeFunctionData("transfer", [TO_ADDRESS]);
    ...
    try {
        ...
        const tx = await daoProposals.createProposal(
            ...
            `Proposal to transfer ${ethers.formatEther(ETH_AMOUNT)} ETH from treasury to specified address`,
            VOTING_DURATION,
            executor, // Use executor as target contract
            executionData
        );
